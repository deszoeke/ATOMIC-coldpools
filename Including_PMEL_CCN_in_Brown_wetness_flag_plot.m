filename = 'EUREC4A_ATOMIC_RonBrown_PMEL_CCN_v2.nc';
ncinfo(filename);
ccn = ncread(filename,'ccn'); % cloud condensation nuclei
cn = ncread(filename,'cn');  % concentration number of particles
ss = ncread(filename,'ss');  % supersaturation
ccn_cn_ratio = ncread(filename,'ccn_cn_ratio'); % ratio of CCN to CN
time_c = ncread(filename,'time'); % time gregorian
time_c = time_c/3600/24 + datenum('19700101','yyyymmdd');
...
scatter(time_p(ind),d18O_eqv(ind),'r','filled')
