...
% Loading iso data
load('iso_data_1min_intervals.mat');
% Incorporating isotope data %
timep = t_avg_1min; % picarro times in 10 min averages
dD   = dD_avg_1min;   % in per mil
d18O = d18O_avg_1min; % in per mil
mr = mr_avg_1min;     % in g/kg
q_iso = (mr/1000)./(1+(mr/1000))*1e3; % air specific humidity [g/kg] from iso data
clearvars d18O_avg_10min dD_avg_10min mr_avg_10min d18O_avg dD_avg mr_avg picarro_times picarro_times_matlab_format t_avg_10min t_avg_10min_matlab_format
iso_de = dD - 8*d18O; % deuterium excess
dD(iso_de<0) = NaN;
q_iso(iso_de<0) = NaN;
iso_de(iso_de<0) = NaN;
...
for k = 1:22
    max_ind(k) = find(round(t_avg_1min,9)==round(t_max2(k),9));
    min_ind(k) = find(round(t_avg_1min,9)==round(t_min2(k),9));
    end_ind(k) = find(round(t_avg_1min,9)==round(t_end2(k),9));
end
...
for k = 1:length(max_ind)
    dummy = t_avg_1min(max_ind(k)-90:end_ind(k));
    tcold(k,1:length(dummy)) = t_avg_1min(max_ind(k)-90:end_ind(k));
...
for k = 1:length(max_ind)
    dummy = t_avg_1min(max_ind(k):min_ind(k));
    tcoldf(k,1:length(dummy)) = t_avg_1min(max_ind(k):min_ind(k));
...
for k = 1:length(max_ind)
    dummy = t_avg_1min(min_ind(k):end_ind(k));
    tcoldw(k,1:length(dummy)) = t_avg_1min(min_ind(k):end_ind(k));
