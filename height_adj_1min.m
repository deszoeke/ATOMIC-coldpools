function [th_ob,q_ob,time_height_adj] = height_adj_1min(zrf,zm,Rd,Cp)
    ...
    time = time/3600/24 + datenum('20200101','yyyymmdd'); % in 10-min resolution
    time_height_adj = time(1):datenum(0,0,0,0,1,0):time(end); % in 1-min resolution
...
%     q_ob = qrf;
    ...
    % Adjusting time resolution of output variables from 10-min to 1-min
    th_ob = interp1(time,th_rf,time_height_adj,'nearest');
    q_ob  = interp1(time,  qrf,time_height_adj,'nearest');
