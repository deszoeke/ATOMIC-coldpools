...
box on
grid on
legend('entrainment','surface','cp front','cp wake','front onset','wake onset','Location','northwest', 'fontsize','12')
legend boxoff;

title(['Cold Pool ',num2str(i),'; onset on ',datestr(tcoldf(i,1))])
% saveas(gcf,['dD_vs_q_CP', sprintf('%02i', i), '_range_RH.png'])
% saveas(gcf,['dD_vs_q_CP', sprintf('%02i', i), '_range_RH.eps'])
end
...
title(['Cold Pool #',num2str(i),'; onset on ',datestr(tcoldf(i,1))])
% saveas(gcf,['d18O_vs_q_CP', sprintf('%02i', i), '_new_ff.png'])
end
