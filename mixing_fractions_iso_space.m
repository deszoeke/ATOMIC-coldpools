...
for i = [2:11,13:16] %[9,14:15] %[8,10,11,13,16] %1:length(max_ind) %[2:7,9,14:15] %1:length(max_ind)-1 %
    ...
    [fen(i,:), fss(i,:), fee(i,:)] = x_y_to_mixfraction(dD_cp,q_cp, dD_en,q_en, dD_surf,q_surf, dD_ev,q_ev);
    [fen_bg(i,:), fss_bg(i,:), fee_bg(i,:)] = x_y_to_mixfraction(dD_bg,q_bg, dD_en,q_en, dD_surf,q_surf, dD_ev,q_ev);
end
%% for iso mixing fraction timeseries plot
% iso_fen(i) = fen(1);
% iso_fss(i) = fss(1);
...
% end
%% Plots for isotopes air mixing fractions %%
