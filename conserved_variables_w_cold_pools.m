...
% flag = 0; % script outputs cold pool plot
flag = 1; % script outputs plot outside of cold pool times
...
% ind0 = find(t_adj==t1min(1:10:end));
...
%% Air type: entrained
% q and theta @ 1km
q_en = q(h==1e3,ind:end)*1e3; % check units for q_inth, looks like is cg/kg
th_en = th(h==1e3,ind:end);

q_en(cold_pool_flag_4hr==flag) = NaN;
th_en(cold_pool_flag_4hr==flag) = NaN;
...
%   theta_w_eqm(p[Pa],theta[K],qv[kg/kg]) from eqn 3.8 of Davies-Jones 2008
    ...
    q_d(cold_pool_flag_4hr==flag) = NaN;
    th_d(cold_pool_flag_4hr==flag) = NaN;
...
%% With iso data %%
conserved_plot_iso(iso_de,th_ob,q_ob,th_en,q_en,th_d,q_d,th_surf,q_surf)
caxis([5 14])

conserved_plot_iso(iso_d18O,th_ob,q_ob,th_en,q_en,th_d,q_d,th_surf,q_surf)
caxis([-12 -9])

conserved_plot_iso(iso_dD,th_ob,q_ob,th_en,q_en,th_d,q_d,th_surf,q_surf)
